...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/Mrs4s/go-cqhttp/extraModule"
	"io"
	"io/ioutil"
	"net/http"
	"net/url"
...
func SelfSign(device *client.DeviceInfo, key string, cmdId string) {
	uin := strconv.FormatInt(base.Account.Uin, 10)

	//baseURL := base.SignServer
	paramsRegister := url.Values{}
	paramsRegister.Add("uin", uin)
	paramsRegister.Add("android_id", string(device.AndroidId))

	guid := strings.ReplaceAll(string(device.BootId), "-", "")

	paramsRegister.Add("guid", guid)
	paramsRegister.Add("qimei36", device.QImei36)
	paramsRegister.Add("key", key)

	// 给代理服务器发送请求
	urlRegister := base.SignServer + "/register?" + paramsRegister.Encode()

	resp, err := http.Get(urlRegister)
	if err != nil {
		log.Errorf("%v", err)
		return
	}
	...
	if code != 0 {
		log.Errorf("%v", data["msg"].(string))
		return
	}

	paramsEnergy := url.Values{}

	paramsEnergy.Add("version", device.Protocol.Version().SdkVersion)
	paramsEnergy.Add("uin", uin)
	paramsEnergy.Add("guid", guid)
	paramsEnergy.Add("data", cmdId)

	// 给代理服务器发送请求
	urlEnergy := base.SignServer + "/energy?" + paramsEnergy.Encode()

	resp, err = http.Get(urlEnergy)
	if err != nil {
		log.Errorf("%v", err)
		return
	}
...
func LoginInteract() {
	...
	if base.SignServer != "-" && base.SignServer != "" {
		...
		SelfSign(device, extraModule.SignCorrelation.SignServerKey, extraModule.SignCorrelation.CmdId)
